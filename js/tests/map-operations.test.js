...
import * as mapOperations from '../map-operations.js'
...
describe("map-operations | getLengthVector", function () {
    beforeEach(() => {
    ...
    it("getLengthVector should be able to return true", function () {
        const map = mapInit.loadMap('map')
        const layers = []
        mapLoader.addLayers(geoConf, map, layers, [{ champ: "code_epci", val: "243500139" }])
        assert.strictEqual(mapOperations.getLengthVector(layers[0]), true)
    })
})